...
namespace PSO_TsaiHung
{
    class Program : PSO
    {
        ...
        static void Main(string[] args)
        {
            ...
            StreamWriter fileWriter = new StreamWriter("GA-23FunctionTest_3.csv");
            ...
            for (int function = 0; function < pso.num_Functions; function++)
            {
                ...
                pso.InitFunction(function + 1);  //呼叫初始函數設定變數內容5
                pso.functionNames[function] = pso.functionName;  //紀錄函數名稱
                pso.best_Solution_Averages[function] = 0;

                pso.processTime = 0;

                double[] tempFunctionAllGlobalBestFitness = new double[pso.num_Repetitive_Runs];

                pso.function_Best_Solutions[function] = Double.MaxValue;

                for (int run = 0; run < pso.num_Repetitive_Runs; run++)
                {
                    pso.Init(50, pso.numDimensions, pso.dimensions_LowerBound, pso.dimensions_UpperBound);
                    pso.Run(160000);
                    tempFunctionAllGlobalBestFitness[run] = pso.GBestFitness;  //紀錄每一次的GBestFitness
                    pso.best_Solution_Averages[function] += pso.GBestFitness;  //將每一次的GBestFitness紀錄下來，後面可用來計算10次的平均

                    //===============================================================================================================

                    if (pso.GBestFitness < pso.function_Best_Solutions[function])
                    {  //紀錄最小(最佳)的GBestFitness
                        pso.function_Best_Solutions[function] = pso.GBestFitness;
                    }
                ...
                pso.processTime = (double)(stopWatch.Elapsed.TotalMilliseconds / 1000);  //紀錄每一個function執行10次的總時間

                //Console.WriteLine("process time: " + ga.processTime);

                pso.best_Solution_Averages[function] /= pso.num_Repetitive_Runs;  //計算平均的GBestFitness

                pso.bestSolutionSDs[function] = sd(tempFunctionAllGlobalBestFitness);  //計算10次GBestFitness的標準差

                pso.function_Average_ProcessTime[function] = pso.processTime / pso.num_Repetitive_Runs;

                Console.WriteLine(pso.functionNames[function] + ", " + pso.function_Best_Solutions[function] + ", " + pso.best_Solution_Averages[function] + ", " + pso.bestSolutionSDs[function] + ", " + pso.function_Average_ProcessTime[function]);

                fileWriter.WriteLine(pso.functionNames[function] + "," + pso.function_Best_Solutions[function] + "," + pso.best_Solution_Averages[function] + "," + pso.bestSolutionSDs[function] + "," + pso.function_Average_ProcessTime[function]);

                //Console.WriteLine("current_resource = " + current_resource);
